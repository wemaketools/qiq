...
(function () {
  ...
  const PRODUCTS = {
    Motor: {
      covers: ["Commercial Fleet", "Private Motor", "Motor Trade"],
      base: 85000,
    },
    Property: {
      covers: [
    ...
    Marine: {
      covers: ["Cargo Insurance", "Marine Hull", "Goods in Transit"],
      base: 410000,
    },
    Liability: {
      covers: [
    ...
    Agriculture: {
      covers: ["Farmers All Risks", "Crop Insurance", "Livestock"],
      base: 450000,
    },
  ...
  const RMS = [
    ...
    { name: "Neo Dlamini", team: "Property & Engineering", skill: 1.1, w: 1.3 },
    {
      name: "Michael Ndlovu",
      team: "Corporate & Commercial",
      skill: 1.05,
    ...
    { name: "Boitumelo Rampa", team: "Agriculture", skill: 0.9, w: 0.9 },
    {
      name: "Lorato Dlamini",
      team: "Property & Engineering",
      skill: 1.02,
      w: 0.9,
    },
  ...
  const BROKERS = [
    ...
    },
    {
      ...
      skill: 0.66,
      w: 0.7,
      contact: "Thabang Dube",
    },
  ];

  // Backward compatibility for persisted demo state with old broker labels.
  const BROKER_NAME_MIGRATION = {
    "Alpha Brokers": "Minet",
    "TradeSure Brokers": "OLEA",
    "Premier Brokers": "Spectrum",
    "Botswana Marine Co.": "Kgare",
    "Minet Botswana": "FinSef",
    "Aon Botswana": "Mogakolodi",
    "AgriPro Brokers": "Chartered",
    "Kalahari Insurance Brokers": "Satib",
    "Northern Agri Brokers": "BOC",
    "Desert Risk Advisors": "Mercantile",
  };
  ...
  function buildClients(rand) {
    const list = HERO_CLIENTS.map((c) =>
      Object.assign({ type: "Corporate" }, c),
    );
  ...
  function generate() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const usesBroker =
        channel === "Broker" || channel === "Tender/RFQ" || rand() < 0.25;
      const broker = usesBroker ? wpick(rand, BROKERS) : null;

      const line = wpick(
        rand,
        LINES.map((l) => ({ l, w: PRODUCTS[l].base > 400000 ? 1.4 : 2.2 })),
        "w",
      ).l;
      const cover = pick(rand, PRODUCTS[line].covers);
      const premium =
        Math.round((PRODUCTS[line].base * lognormalFactor(rand)) / 100) * 100;
      ...
      if (rand() < pClosed) {
        // decided
        const pWin = clamp(0.3 * skill, 0.12, 0.62);
        ...
        if (rand() < pWin) {
          ...
          lossReason = wpick(
            ...
            [
              { v: "Pricing", w: 3.2 },
              { v: "Incumbent retained", w: 2.2 },
              { v: "Tender outcome", w: 1.6 },
              { v: "Brand trust", w: 1.0 },
              { v: "Coverage gaps", w: 1.3 },
              { v: "Service concerns", w: 1.0 },
              { v: "Other", w: 0.7 },
            ],
            "w",
          ).v;
          if (
            lossReason === "Pricing" ||
            lossReason === "Incumbent retained" ||
            rand() < 0.4
          ) {
            competitor = pick(rand, COMPETITORS);
            if (lossReason === "Pricing") {
              competitorPremium = Math.round(
                premium * smooth(rand, 0.72, 0.95),
              );
            }
          }
          lossComment =
            lossReason === "Pricing"
              ? "Client cited premium ~" +
                Math.round(smooth(rand, 6, 22)) +
                "% above competing quote."
              : lossReason === "Incumbent retained"
                ? "Incumbent insurer matched terms at renewal."
                : lossReason === "Tender outcome"
                  ? "Tender awarded to alternate panel member."
                  : "";
        }
      } else {
        // open
      ...
      if (reachedSent && sent <= TODAY) {
        ...
        for (let k = 0; k < n && cur <= (decisionDate || TODAY); k++) {
          followUps.push({
            ...
            by: rm.name,
            note: pick(rand, [
              "Called broker for update",
              "Emailed revised terms",
              "Client requested clarification on cover",
              "Awaiting signed acceptance",
              "Left voicemail with contact",
              "Sent reminder ahead of expiry",
            ]),
          });
          lastFollowUp = cur;
          followUpCount++;
      ...
      if (reachedIdx >= 2 || status !== "open") {
        ...
        if (reachedIdx >= 3 || status !== "open") {
          uwReturned = addDays(uwAssigned, uwDur);
        }
        const dur = uwReturned
          ? daysBetween(uwAssigned, uwReturned)
          : daysBetween(uwAssigned, TODAY);
      ...
      const q = {
        ...
        datePrepared:
          reachedIdx >= 2 || status !== "open" ? prepared.getTime() : null,
        ...
        flags,
        followUps,
        lastFollowUp: lastFollowUp ? lastFollowUp.getTime() : null,
        nextFollowUp: nextFollowUp ? nextFollowUp.getTime() : null,
        followUpCount,
        underwriter,
        uwAssigned: uwAssigned ? uwAssigned.getTime() : null,
        uwReturned: uwReturned ? uwReturned.getTime() : null,
        slaStatus,
        ...
        lossComment,
        createdBy: rm.name,
        updatedAt: (decisionDate || lastFollowUp || sent || received).getTime(),
        notes: "",
      };
      ...
      if (status === "open") {
        ...
        if ((stage === "proposal" || stage === "negotiation") && rand() < 0.4)
          flags.push("awaiting_broker");
  ...
  const D = {
    ...
    age(q) {
      return daysBetween(new Date(q.dateReceived), TODAY);
    },
    turnaround(q) {
      return q.dateSent
        ? daysBetween(new Date(q.dateReceived), new Date(q.dateSent))
        : null;
    },
    decisionDays(q) {
      return q.dateSent && q.decisionDate
        ? daysBetween(new Date(q.dateSent), new Date(q.decisionDate))
        : null;
    },
    uwDays(q) {
      return q.uwAssigned
        ? daysBetween(
            new Date(q.uwAssigned),
            q.uwReturned ? new Date(q.uwReturned) : TODAY,
          )
        : null;
    },
    isOpen(q) {
    ...
    isHighValue(q) {
      return q.premium >= D.HIGH_VALUE;
    },
    isExpired(q) {
      return (
        q.status === "open" && q.validUntil && new Date(q.validUntil) < TODAY
      );
    },
    isExpiring(q) {
      return (
        ...
        q.validUntil &&
        !D.isExpired(q) &&
        daysBetween(TODAY, new Date(q.validUntil)) <= 7
      );
    },
    isOverdue(q) {
      return (
        q.status === "open" &&
        q.nextFollowUp &&
        new Date(q.nextFollowUp) < TODAY
      );
    },
    isStalled(q) {
      return (
        q.status === "open" &&
        D.daysSinceActivity(q) >= 7 &&
        (q.reachedIdx >= 3 || D.isHighValue(q))
      );
    },
    daysSinceActivity(q) {
      return daysBetween(
        new Date(q.lastFollowUp || q.dateSent || q.dateReceived),
        TODAY,
      );
    },
    isAtRisk(q) {
    ...
    isUWOverSLA(q) {
      return q.slaStatus === "breached" && q.status === "open";
    },

    stageLabel(s) {
      return (STAGES[s] || { label: s }).label;
    },

    byId(id) {
      return D.quotes.find((q) => q.id === id);
    },
    ...
    migrateQuotes(quotes) {
      return quotes.map((q) => {
        if (!q || !q.broker) return q;
        const migrated = BROKER_NAME_MIGRATION[q.broker];
        return migrated ? { ...q, broker: migrated } : q;
      });
    },
    load() {
      try {
        ...
        if (raw) {
          ...
          if (obj && obj.quotes && obj.quotes.length) {
            D.quotes = D.migrateQuotes(obj.quotes);
            return;
          }
        }
      } catch (e) {}
    ...
    save() {
      try {
        localStorage.setItem(D.KEY, JSON.stringify({ quotes: D.quotes, v: 1 }));
      } catch (e) {}
    },
    reset() {
      D.quotes = generate();
    ...
    nextId() {
      let max = 1000;
      D.quotes.forEach((q) => {
        const n = parseInt(String(q.id).slice(-4), 10);
        if (n > max) max = n;
      });
    ...
    addQuote(data) {
      ...
      const q = Object.assign(
        {
          id: D.nextId(),
          ref: "BRT/2025/" + Math.floor(5000 + Math.random() * 4000),
          stage: "new",
          status: "open",
          reachedIdx: 0,
          flags: [],
          followUps: [],
          lastFollowUp: null,
          nextFollowUp: addDays(TODAY, 2).getTime(),
          followUpCount: 0,
          dateReceived: now,
          dateAssigned: now,
          datePrepared: null,
          ...
          competitorPremium: null,
          lossComment: "",
          createdBy: (QIQ.state && QIQ.state.user.name) || "System",
          updatedAt: now,
          notes: "",
          history: [
            {
              ts: now,
              action: "Quote request captured",
              by: (QIQ.state && QIQ.state.user.name) || "System",
            },
          ],
        },
        data,
    ...
    advance(q) {
      ...
      q.stage = next;
      q.reachedIdx = Math.max(q.reachedIdx, idx + 1);
      if (next === "pricing" && !q.datePrepared) q.datePrepared = now;
      if (next === "proposal") {
        q.dateSent = q.dateSent || now;
        q.validUntil = q.validUntil || addDays(TODAY, 30).getTime();
      }
      D._log(q, "Advanced to " + STAGES[next].label);
      D.save();
      return q;
    },
    markWon(q) {
      ...
      q.stage = "won";
      q.reachedIdx = 5;
      q.decisionDate = TODAY.getTime();
      q.boundPremium = q.premium;
      q.nextFollowUp = null;
      D._log(q, "Marked Won — premium bound");
      D.save();
      return q;
    },
    markLost(q, reason, competitor, compPremium, comment) {
      ...
      q.decisionDate = TODAY.getTime();
      q.lossReason = reason;
      q.competitor = competitor || null;
      q.competitorPremium = compPremium || null;
      q.lossComment = comment || "";
      q.nextFollowUp = null;
      D._log(q, "Marked Lost — " + reason);
      D.save();
      return q;
    },
    logFollowUp(q, note) {
      ...
      q.followUps.push({
        date: now,
        by: (QIQ.state && QIQ.state.user.name) || "RM",
        note,
      });
      q.lastFollowUp = now;
    ...
    filter(f) {
      ...
      return D.quotes.filter((q) => {
        ...
        if (f.broker && f.broker !== "All" && q.broker !== f.broker)
          return false;
        if (f.rm && f.rm !== "All" && q.rm !== f.rm) return false;
        if (f.region && f.region !== "All" && q.region !== f.region)
          return false;
        if (f.status && f.status !== "All" && q.status !== f.status)
          return false;
        if (f.stage && f.stage !== "All" && q.stage !== f.stage) return false;
        if (f.channel && f.channel !== "All" && q.channel !== f.channel)
          return false;
        if (f.month != null) {
          const d = new Date(q.dateReceived);
          if (d.getMonth() !== f.month.m || d.getFullYear() !== f.month.y)
            return false;
        }
        if (f.search) {
  ...
  QIQ.fmt = {
    ...
    bwp(n, compact) {
      return "BWP " + QIQ.fmt.money(n, compact);
    },
    pula(n, compact) {
      return "P " + QIQ.fmt.money(n, compact);
    },
    pct(n, d) {
